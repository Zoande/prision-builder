// Room border pass: always-on thin strips around every room, colored by
// access (see rooms.wgsl). Instance data comes from World.roomOutline().

import roomsShader from "../rooms.wgsl?raw";
import { PRELUDE } from "./shaderCommon";

export class RoomLinePass {
  ...
  static create(
    ...
    const p = new RoomLinePass();
    const module = device.createShaderModule({ code: PRELUDE + roomsShader });
