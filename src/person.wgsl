...
fn vs(
  ...
  let pid = part - floor(part / 8.0) * 8.0;
  ...
  if (pid > 5.5 && pid < 6.5 && baton < 0.5) { p = vec3<f32>(0.5, 0.7, 0.5); }
  if (pid > 6.5 && !carrying) { p = vec3<f32>(0.5, 0.7, 0.5); }

  let isLeg = pid > 3.5 && pid < 5.5; // legs + shoes
  let isArm = pid > 2.5 && pid < 3.5;
  ...
  var out : VSOut;
  out.world = vec3<f32>(posxz.x + q.x, q.y, posxz.y + q.z);
